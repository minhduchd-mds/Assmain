...
//        ConvertData cv = new ConvertData();
        String sql = "INSERT INTO APP.Employce(LastName, FirstName, birthday, HireDay, Address, City, Country, "
...
//            ps.setString(4, cv.date2string(e.getHireDate(), ValidData.EN_DATE));
    ...
    public int delete(Employee e) {
        String sql = "DELETE FROM APP.Employce WHERE employeeid=?";
        try {
            ps = dao.getConnection().prepareStatement(sql);
...
//        ConvertData cv = new ConvertData();
        String sql = "UPDATE APP.Employce SET LastName=?, FirstName=?, BirthDay=?, HireDay=?, Address=?, City=?, Country=?,"
...
//            ps.setString(4, cv.date2string(e.getHireDate(), HRManager.ValidData.EN_DATE));
    ...
    public List<Employee> select() {
        String sql = "select * from APP.Employce";
//        DAO dao = new DAO();
        ...
        try {
            ...
            while (rs.next()) {
                ...
                e.setHireDate(rs.getString("HireDay"));
    ...
    public List<Employee> find(int option, String value) {
        ...
        switch (option) {
            case 0:
                sql = "select * from APP.Employce where firstname=? or lastname=?";
                ...
                break;
            case 1:
                sql = "select * from APP.Employce where city=?";
                try {
                    ps = dao.getConnection().prepareStatement(sql);
                    ps.setString(1, value);
                } catch (SQLException ex) {
...
//        DAO dao = new DAO();
    ...
    public Employee getByID(String id) {
        String sql = "select * from APP.Employce where employeeid=?";
        try {
            ps = dao.getConnection().prepareStatement(sql);
